...
def _extract_nia_period_from_detail(html: str) -> str:
    ...
    for keyword in ("접수기간", "공고기간", "제출기간", "마감일시", "마감"):
...
def crawl_kisa(start_page: int, end_page: int, org: str = "KISA", delay_range=(0.5, 1.0)) -> list[Announcement]:
    ...
    for page in range(start_page, end_page + 1):
        ...
        for idx, item in enumerate(items):
            if not item.상세링크:
                continue
            if not (item.접수시작일 or item.접수종료일):
                try:
            ...
            # 접수시작일을 끝내 못 찾으면 공고일자로 대체(요청사항)
            if not items[idx].접수시작일 and items[idx].공고일자:
                items[idx] = replace(items[idx], 접수시작일=items[idx].공고일자)
