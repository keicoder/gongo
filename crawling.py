...
def crawl_smes(delay_range=(0.5, 1.0)) -> list[Announcement]:
    token = os.environ.get("SMES_API_KEY")
    if not token:
        print("[SMES] SMES_API_KEY 환경변수가 없어 건너뜁니다", file=sys.stderr)
        return []

    print("[SMES] 공고정보 API 조회 중...", file=sys.stderr)
    try:
        resp = requests.get(
            SMES_API_URL,
            params={"token": token, "html": "no"},
    ...
    print(f"[DEBUG] SMES_API_KEY exists={token is not None}, length={len(token) if token else 0}")

    if not token:
        print("[SMES] SMES_API_KEY 환경변수가 없어 건너뜁니다")
