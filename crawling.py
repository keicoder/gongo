...
def update_db(items: list[Announcement], db_path: str) -> int:
    ...
    if "작성일자" in merged.columns:
        merged["작성일자"] = merged["작성일자"].fillna("").apply(_normalize_written_date)
        merged = merged.sort_values("작성일자", ascending=False, kind="stable")
