...
def enrich_announcement(ann: Announcement) -> Announcement:
    ...
    기관명 = ann.기관명
    if ann.상세링크.startswith("https://www.bizinfo.go.kr"):
        기관명 = "MSS"
...
def fetch_nia_detail(상세링크: str) -> str:
    ...
    for _ in range(3):
        try:
            resp=requests.get(상세링크, headers=HEADERS, timeout=20)
            break
        except requests.RequestException as e:
            last_error=e
            time.sleep(2)
...
def crawl_smes(delay_range=(0.5, 1.0), lookback_days: int = 90) -> list[Announcement]:
    ...
    print(f"[SMES] resultCd={payload.get('resultCd')}, resultMsg={payload.get('resultMsg')}", file=sys.stderr)
