...
class Announcement:
    공고일자: str = ""  # 가장 앞쪽 열
    기관명: str = ""
    # 아래는 크롤링 시점에 제목/비고/신청기간을 분석해 자동으로 채우는 파생 컬럼
...
# ---------------------------------------------------------------------------
# AI품질역량센터 관점 자동 분류 (제목/비고/신청기간 기반 규칙)
# ---------------------------------------------------------------------------
...
def _compact_loose_dates(text: str) -> str:
    return _LOOSE_DATE_RE.sub(lambda m: f"{m.group(1)}.{m.group(2)}.{m.group(3)}", text)
...
def enrich_announcement(ann: Announcement) -> Announcement:
    """제목/비고/신청기간을 분석해 AI품질역량센터 관점의 분류 컬럼을 채운 새 Announcement 반환."""
...
def crawl_kiat(start_page: int, end_page: int, org: str = "KIAT", delay_range=(0.5, 1.0)) -> list[Announcement]:
    ...
    for page in range(start_page, end_page + 1):
        ...
        # 목록에서 신청기간을 못 얻은 경우, 상세페이지의 '접수기간 / 상태'에서 보완
        for idx, item in enumerate(items):
            if item.신청기간 or not item.상세링크:
                continue
            try:
                detail_html = fetch_kiat_detail(item.상세링크)
                period_text = _extract_kiat_period_from_detail(detail_html)
                if period_text:
                    items[idx] = replace(item, 신청기간=period_text)
            except requests.RequestException as e:
...
def _extract_kisa_period_from_detail(html: str) -> str:
    ...
    for keyword in ("접수기간", "제출기간", "신청기간", "마감일시", "마감"):
        idx = body_text.find(keyword)
        if idx == -1:
            continue
        window = body_text[idx: idx + 80]
...
def crawl_kisa(start_page: int, end_page: int, org: str = "KISA", delay_range=(0.5, 1.0)) -> list[Announcement]:
    ...
    for page in range(start_page, end_page + 1):
        ...
        # 목록에 신청기간이 없으므로, 상세페이지 본문에서 마감/접수기간 문구를 찾아 보완
        for idx, item in enumerate(items):
            if item.신청기간 or not item.상세링크:
                continue
            try:
...
def crawl_iris(start_page: int, end_page: int, delay_range=(0.5, 1.0)) -> list[Announcement]:
    ...
    for page in range(start_page, end_page + 1):
        ...
        # 목록에 신청기간이 없으므로, 상세페이지의 '접수기간' 항목에서 보완
        for idx, item in enumerate(items):
            if item.신청기간 or not item.상세링크:
                continue
            try:
